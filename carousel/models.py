...
class GalleryPlugin(CMSPlugin):
    ...
    def __unicode__(self):
        if self.image_set.count() == 1:
            return _(u'%(count)d image in gallery') % {'count': self.image_set.count()}
        else :
            return _(u'%(count)d images in gallery') % {'count': self.image_set.count()}
...
class Image(Orderable):
    ...
    src = FilerImageField(null=True, blank=True, related_name= "gallery_image")
