...
def eventDetailView(request, event_slug):
    events= Event.objects.filter(slug= event_slug)
    event= events[0]
    event_set= event.recurringevent_set.all()
    eventItemDict= {'slug':event_slug, 'event': event, 'event_set': event_set,}
    return render_to_response('events/event_detail.html',eventItemDict, context_instance=RequestContext(request))
