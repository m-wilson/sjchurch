...
def sorted_event_list(start_date=None, end_date=None, limit=None, specified_categories= None):
    ...
    if specified_categories is None:
            ...
            if event.recurringevent_set.all():
                ...
                for i in event.recurringevent_set.all():
                    ...
                    for j in event_ruleset.between(start_date_naive, end_date_naive):
                        ...
                        event_list.append((event_occurance_tz, event, DAY_CHOICES[event.start.weekday()][1], event.end.time()))
            else:
                # One off event
        ...
        for event in Event.objects.filter(category__eventcategory__category__in=specified_categories).prefetch_related('recurringevent_set', 'recurringeventexclusion_set').distinct():
            if event.recurringevent_set.all():
                # Recurring event
                for i in event.recurringevent_set.all():
                    event_ruleset = rrule.rruleset()
